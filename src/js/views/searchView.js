...
export const renderTitle = (query, numResults) => {
    ...
    document.querySelector(domStrings.searchResultTitle).insertAdjacentHTML('beforeend', titleHTML);
}
...
const renderItem = item => {
    ...
    const item_HTML = `
        <a href="recipe.html?rID=${item.recipe_id}&return=true" class="search-results__link" href="${item.recipe_id}">
            <figure class="search-results__item">
                <img src="${item.image_url}" class="search-results__img">
            </figure>
    ...
    document.querySelector(domStrings.searchResultGrid).insertAdjacentHTML('beforeend', item_HTML);
}
